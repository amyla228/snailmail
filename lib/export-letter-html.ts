/**
 * Builds a self-contained HTML file: closed envelope → click → open → letter unfolds.
 * Inline CSS/JS so it works standalone. Used by envelope "Download Letter".
 */
...
function letterContentHtml(letter: SavedLetterState): string {
  ...
  const pagesHtml = pages
    ...
    .join("")
  const decosHtml = (letter.decorations || []).map(decorationToHtml).join("")
  return `
